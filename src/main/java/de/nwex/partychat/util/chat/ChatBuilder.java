...
import de.nwex.partychat.util.TextUtil;
...
public class ChatBuilder {
    ...
    /*
    public static MutableText player(MutableText text, UUID uuid) {
        return chain(text, () -> text.getStyle().withHoverEvent(new ClientPlayerRenderHoverEvent(uuid).create()));
    }
     */
    ...
    public static MutableText run(MutableText text, Runnable runnable) {
        return chain(text, () -> text.getStyle().withClickEvent(new ClientRunnableClickEvent(runnable).create()));
    }
     */
